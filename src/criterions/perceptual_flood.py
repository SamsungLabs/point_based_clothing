...
class Criterion(nn.Module):
    ...
    def forward(self, inputs):
        ...
        lossmask = (flood_mask + real_segm).clamp(0., 1.).detach()

        fake_rgb = to_tanh(to_sigm(fake_rgb) * lossmask)
        real_rgb = to_tanh(to_sigm(real_rgb) * lossmask)

        # out_dict = dict(fake_rgb=fake_rgb, real_rgb=real_rgb, real_segm=real_segm, flood_mask=flood_mask, lossmask=lossmask)
